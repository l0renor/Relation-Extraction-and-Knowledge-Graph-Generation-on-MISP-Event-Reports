...
def get_metrics(relations_actual, relations_expected):
    ...
    for relation_actual in relations_actual:
        for relation_expected in relations_expected:
            if (relation_expected['head']['text'] == relation_actual['head']['text']) and \
                    (relation_expected['head']['label'] == relation_actual['head']['label']) and \
                    (relation_expected['subj']['text'] == relation_actual['subj']['text']) and \
    ...
    for relation_true in true_positive:
        for relation_actual in relations_actual:
            if relation_true['head']['text'] == relation_actual['head']['text']  and \
                    relation_true['head']['label'] == relation_actual['head']['label'] and  \
                    relation_true['subj']['text'] == relation_actual['subj']['text'] and \
            relation_true['subj']['label'] == relation_actual['subj']['label'] and \
            relation_true['rel'] == relation_actual['rel']:
...
def process_line(row, model, tokenizer):
    expected_relations = parse_model_output(row["text_label"])
